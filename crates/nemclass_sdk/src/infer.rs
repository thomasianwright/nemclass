//! Heuristic type inference: given a window of bytes read from the target, guess
//! the most likely [`FieldKind`]. Powers the GUI's "Guess type" action, the live
//! Hex-view hints, and the scripting `nem.infer` binding.

use crate::target::Target;
use crate::types::{FieldKind, FloatWidth};
...
/// Reads up to [`WINDOW`] bytes at `address`, shrinking to 8 if the wider read
/// fails. Returns an empty vec if nothing could be read.
pub fn read_window(target: &Target, address: usize) -> Vec<u8> {
    let mut buf = [0u8; WINDOW];
    if target.read(address, &mut buf) {
        return buf.to_vec();
    }
    let mut small = [0u8; 8];
    if target.read(address, &mut small) {
        return small.to_vec();
    }
    vec![]
}

/// True if the little-endian f32 at the start of `bytes` looks like a real value
/// rather than random bits: normal (no zero/NaN/inf/denormal) and within a sane
/// magnitude band.
...
/// Counts how many consecutive `lane`-byte groups from the start of `bytes`
/// satisfy `plausible`.
...
fn points_to_string(target: &Target, addr: usize) -> bool {
    let mut buf = [0u8; 16];
...
/// Returns candidate [`FieldKind`]s for `bytes`, best first. Empty if nothing
/// beats raw bytes.
///
/// Priority: valid pointer (string pointer preferred) > multi-lane float vector
/// (more lanes / f32 first) > scalar f64 > scalar f32. Conservative on purpose so
/// live hints stay low-noise.
pub fn infer_kind(bytes: &[u8], target: &Target) -> Vec<FieldKind> {
    let mut out = vec![];

    // Pointer — strongest signal. Read the target's pointer width.
    let ptr = target.pointer_size().min(8);
    if bytes.len() >= ptr {
        let mut raw = [0u8; 8];
        raw[..ptr].copy_from_slice(&bytes[..ptr]);
        let v = usize::from_ne_bytes(raw);
        if v >= MIN_POINTER && target.can_read(v) {
            if points_to_string(target, v) {
...
/// The best inferred kind that is a float or vector (skips pointers, which the
/// Hex view already signals via its pointer column). Used for low-noise live hints.
pub fn infer_float_hint(bytes: &[u8], target: &Target) -> Option<FieldKind> {
    infer_kind(bytes, target).into_iter().find(|k| {
        matches!(
            k,
...
mod tests {
    ...
    fn float_plausibility() {
        ...
        assert!(!plausible_f32(&5u32.to_ne_bytes()));
